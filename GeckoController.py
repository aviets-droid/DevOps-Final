...
class GeckoController:
    ...
    def newCollection(self):
        try:
            conn = psycopg2.connect("dbname=LeopardGeckos user=postgres password=#2Truckee port=5433")
        except:
            print("Database failed to connect.\n")
        
        cur = conn.cursor()
        try:
            cur.execute("""CREATE TABLE IF NOT EXISTS usercollection (
    ...
    def fetchMorph(self, morphName, tablename):
        ...
        try:
            cur.execute(f"""SELECT name, type, issue, description FROM "{tablename}" LEFT JOIN "HealthInfo" ON "{tablename}".name = "HealthInfo".morph WHERE name = '{morphName}';""")
        except:
            return -1
